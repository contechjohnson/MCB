-- MCB Database Schema v2.1
-- Flexible matching architecture for multi-source attribution tracking
-- Created: Nov 3, 2025
--
-- KEY DESIGN DECISIONS:
-- - UUID primary key (allows creation before we have GHL_ID or MC_ID)
-- - All external IDs are UNIQUE but NULLABLE (contacts enter at different points)
-- - Email-based matching for linking contacts across platforms
-- - Phone is optional (formatting nightmare across platforms)
...
-- ====================
-- STEP 2: Create new contacts table with flexible matching
-- ====================
...
CREATE TABLE contacts (
  ...
  stage TEXT,                          -- Current funnel stage (auto-updated by webhooks)
                                       -- Possible values: new_lead, DM_qualified, landing_link_sent,
                                       -- landing_link_clicked, form_submitted, meeting_booked,
                                       -- meeting_held, purchased, feedback_requested, feedback_received
...
CREATE TABLE webhook_logs (
  ...
  event_type TEXT,                     -- Type of event (e.g., 'dm_qualified', 'booking_created')
  ...
  -- Foreign key to contacts (optional, for tracking)
  CONSTRAINT fk_contact FOREIGN KEY (contact_id) REFERENCES contacts(id) ON DELETE SET NULL
);

CREATE INDEX idx_webhook_logs_source ON webhook_logs(source);
CREATE INDEX idx_webhook_logs_contact_id ON webhook_logs(contact_id) WHERE contact_id IS NOT NULL;
CREATE INDEX idx_webhook_logs_mc_id ON webhook_logs(MC_ID) WHERE MC_ID IS NOT NULL;
CREATE INDEX idx_webhook_logs_ghl_id ON webhook_logs(GHL_ID) WHERE GHL_ID IS NOT NULL;
CREATE INDEX idx_webhook_logs_created_at ON webhook_logs(created_at);
CREATE INDEX idx_webhook_logs_status ON webhook_logs(status);

-- ====================
-- STEP 5: Create stripe events table
-- ====================

CREATE TABLE stripe_events (
  id BIGSERIAL PRIMARY KEY,
  event_id TEXT UNIQUE NOT NULL,       -- Stripe event ID (prevents duplicate processing)
  event_type TEXT NOT NULL,            -- 'checkout.session.completed', 'charge.refunded', etc.
  customer_email TEXT,                 -- Email from Stripe customer
  contact_id UUID,                     -- Matched contact (if found)
  MC_ID TEXT,                          -- Matched ManyChat ID (if applicable)
  GHL_ID TEXT,                         -- Matched GHL ID (if applicable)
  amount DECIMAL(10, 2),               -- Amount in dollars
  status TEXT,                         -- 'paid', 'refunded', 'failed'
  raw_event JSONB,                     -- Full Stripe event JSON for debugging
  created_at TIMESTAMPTZ DEFAULT NOW(),

  -- Foreign key to contacts
  CONSTRAINT fk_stripe_contact FOREIGN KEY (contact_id) REFERENCES contacts(id) ON DELETE SET NULL
);

CREATE INDEX idx_stripe_events_email ON stripe_events(customer_email);
CREATE INDEX idx_stripe_events_contact_id ON stripe_events(contact_id) WHERE contact_id IS NOT NULL;
...
-- ====================
-- STEP 6: Helper functions for smart matching
-- ====================
...
-- ====================
-- SUCCESS! Schema v2.1 is ready.
-- ====================

-- USAGE EXAMPLES:
--
-- 1. Find contact by any identifier:
--    SELECT * FROM contacts WHERE id = find_contact_smart(
--      search_ghl_id := 'ghl_123',
--      search_mc_id := 'mc_456',
--      search_email := 'test@example.com'
--    );
--
-- 2. Find by email only:
--    SELECT * FROM contacts WHERE id = find_contact_by_email('test@example.com');
--
-- 3. Create ManyChat contact (early funnel):
--    INSERT INTO contacts (MC_ID, email_primary, subscribe_date, stage)
--    VALUES ('mc_12345', 'test@example.com', NOW(), 'new_lead');
--
-- 4. Update when they book (add GHL_ID):
--    UPDATE contacts
--    SET GHL_ID = 'ghl_67890', meeting_book_date = NOW(), stage = 'meeting_booked'
--    WHERE id = find_contact_by_email('test@example.com');
